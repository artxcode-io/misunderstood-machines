...
const hlStudio = (function (hl) {
  ...
  hl.random = function (...args) {
    ...
    const rand = sfc32(originalReferences.randomSeed(), originalReferences.randomSeed(), originalReferences.randomSeed(), originalReferences.randomSeed())();
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.eventName === "HARVEST_TRAIT") {
      ...
      originalReferences.randomSeed = originalReferences.context.isCurated ? xmur3(tx.hash) : xmur3(tx.hash + tx.tokenId);
      const newHl = {
        ...hl,
        tx,
      };
